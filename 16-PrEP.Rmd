...
-   **Newly on PrEP (FY24)** $PrEP\_NEW.T$
...
3.  Review data pulled from DATIM showing "PrEP_NEW (FY22 Results)", and
    "PrEP_NEW (FY23 Targets)".

4.  Manually enter targets for "Newly on PrEP (FY24)".
...
2.  Manually enter targets for "PrEP_CT (FY24)".
...
-   **PrEP_CT - # HIV tests administered (FY24)** $PrEP\_CT.TestResult.T$
...
1.  The number of HIV Tests administered for PrEP_CT will be calculated by multiplying
    ...
    target for column Q. 
2.  This target for FY24 PrEP_CT - # HIV tests administered will then pull directly 
    into the HTS tab in which PrEP_CT tests will all fall under the roll-up for 
    TotalHTS_TST_NEG (FY24).
