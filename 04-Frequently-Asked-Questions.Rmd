...
A: Users will see three (3) columns for PLHIV in the Cascade and Year 2 tabs. In some cases, teams will receive two different PLHIV Estimates, one for "PLHIV_Residents.T" and another for "PLHIV_Attend.T". Please refer back to the Cascade section in the  [Partner Country Context] portion. The default is to point to column AK, but if the tool only has values in column AL, the formula will need to be updated as specified in the note in you hover over cell AL3. Once you make this change and drag down the formula and you have PLHIV in column AL, and re-validate and send to PAW, PLHIV should appear.

***Q: On the PMTCT Tab, the Spectrum Estimate for column Q "P.C. ANC Clients - New on ART (FY23)" is not pulling from the Spectrum estimates tab. What needs to be done for this to pull into the PMTCT Tab?***

A: In the formula to pull from the Spectrum tab there is a reference to "CY2022Q3". This needs to be changed starting in Row 15 (Cell Q15) to read "CY2023Q3". Once this is updated, the formula will need to be dragged down through the entire column Q so that it properly updates and pulls estimates throughout the entire column.

***Q: On the PMTCT Tab, the Spectrum Estimate for column L "P.C. ANC Clients - Known Positive (FY23)" is not pulling from the Spectrum estimates tab. What needs to be done for this to pull into the PMTCT Tab?***

A: In the spectrum estimates, there is a syntax error in the indicator code that is causing the estimates to not populate. The indicator code in the PMTCT Tab is "PMTCT_STAT_SUBNAT.N.KnownPos.T_1" whereas the Spectrum estimates are using "PMTCT_STAT_SUBNAT.N.Known.Pos.T_1". The inclusion of the "." in "Known.Pos" is why the estimates are not populating. Teams should edit the spectrum estimates to remove the period "." and ensure it reads "PMTCT_STAT_SUBNAT.N.KnownPos.T_1". Once these indicator codes in Column G of the Spectrum tab are updated they will pull into the PMTCT Tab.

***Q: On the TX_TB_PREV Tab, the FY23 TX_CURR Targets and FY24 TX_NEW Targets are not aggregating the \<15 and 15+ age bands properly, how can we address this?***

A: Important Note, if this issue is present in your tool, it may be your version of excel, or in the generation of tools that excel added a "\@", that caused this to appear in any "IFERROR" formula in the Target Setting Tool. If you see this in any other "IFERROR" formula, please feel free to follow the step to resolve. In both of the formulas in columns F and G of the TX_TB_PREV tab there is an "\@" symbol halfway through the formula. This needs to be removed in order for the formula to work properly. You will need to delete the "\@", and then drag this formula down through both of these columns to have the aggregation work as intended.
