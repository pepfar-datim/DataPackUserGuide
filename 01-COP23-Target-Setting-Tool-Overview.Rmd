...
past several years, the Target Setting Tool has a been a key element of
PEPFAR COP planning, and for COP23 serves a critical function in
...
indicators included in the Target Setting Tool are **MER 2.6**
...
prioritization levels for COP23. The COP23 Target Setting Tool assumes a
'test and start' treatment platform and will develop targets for
achieving 95% coverage in Scale up: Aggressive and Scale-up: Saturation
PSNUs; all other targets in the Target Setting Tool are based on the
treatment targets, insofar as the treatment targets are the main focus
of reaching epidemic control, and therefore relate to both testing and
prevention targets.
...
The COP23 Target Setting Tool is largely the same as the COP21 Target
Setting Tool. However, please note the following updates that have been
implemented as a result of multiple feedback sessions with various
country teams that had been identified by the PRIME team, as well as new
programmatic changes that are reflected in the Section 7 of COP
guidance. These changes revolve around workflow, ease of target setting,
and linkage to the COP guidance based on different aspects of the Target
Setting Tool that worked well and others that did not during COP21
target stetting:
...
produce FY22 targets that will be subsequently submitted through DATIM
...
-   Country Teamspre-validates their Target Setting Tool submission in
    the Target Setting Tool Self-Service App (available at
    <https://apps.datim.org/datapack/>).

-   Country Team uses Target Setting Tool Self-Service App to sync data
    with PAW Dossiers.

-   Country Team saves Target Setting Tool to SharePoint under the OU's
    HQ Collaboration \> COP 2022 - FY 2023 \> Guidance, Tools, and
    Resources folder.
...
-   Country Team submits a ticket in ZenDesk that includes:
    ...
    -   Confirmation that this file has been pre-validated in the Target
        Setting Tool (DataPack) Self-Service App

    -   Confirmation that this file has been sent to PAW via the Target
        Setting Tool (DataPack) Self-Service App
...
tab will appear as a blank sheet, but will be generated by the
self-service validation app after you submit your preliminary Target
Setting Tool.
...
4.  Once changes are aprroved, either through an OPU for targets, or
    ...
    values into the related column of the Target Setting Tool yourself.
    If you wish to request a new Target Setting Tool, you may do so, but
    will have to start the Target Setting Tool process afresh. For
    either of these routes, reach out to the Target Setting Tool Systems
    Team via Zendesk for support.
