...
-   **Host Country Estimated PLHIV who Know HIV Status (FY22)
    \[DIAGNOSED_SUBNAT.T_1\]**: Estimated PLHIV who know their HIV
...
-   **Host Country Estimated Virally Suppressed ART Patients
    ...
    virally suppressed, projected as of September 2021.
...
## Cascade: PopVLS_NET_NEW_SUBNAT

```{r echo=FALSE, include=knitr::is_latex_output()}
schema_table("Cascade-VLS_NET_NEW_SUBNAT-AO-AT-1-3", "latex")

schema_table("Cascade-VLS_NET_NEW_SUBNAT-AO-AT-2-3", "latex")

schema_table("Cascade-VLS_NET_NEW_SUBNAT-AO-AT-3-3", "latex")
```

```{r echo=FALSE, include=knitr::is_html_output()}
schema_table("Cascade-VLS_NET_NEW_SUBNAT-AO-AT-1-1", "html")
```
...
-   **Targeted Host Country VL_SUPPRESSION_SUB NAT (FY23)
    \[**VL_SUPPRESSED.T\]
-   **Targeted Host Country VLS_NET_NEW_SUBNAT (FY23)**
    \[VLS_NET_NEW_SUBNAT.T\]
...
-   **TX_PVLS_NET_NEW (FY23)** \[TX_PVLS_NET_NEW.N.T\]
...
```{r echo=FALSE, include=knitr::is_latex_output()}
schema_table("Cascade-HTS_INDEX-CF-CM-1-1", "latex")

schema_table("Cascade-HTS_INDEX-CF-CM-2-2", "latex")
```

```{r echo=FALSE, include=knitr::is_html_output()}
schema_table("Cascade-HTS_INDEX-CF-CM-1-1", "html")
```
