...
# COP23 Health Equity Analysis and Targeting Tool (HEATT) Overview

Welcome to the COP23 HEATT User Manual. The following pages aim to
provide users of the HEATT with the information necessary to
successfully complete each tab of the HEATT tool and determine
accurate, data-driven targets. For the past several years, the HEATT
...
HEATT is mandatory and must be used to set targets for COP23. For
COP23, all indicators included in the HEATT are **MER 2.6**
...
The COP23 HEATT supports analysis for all targets by Priority
...
prioritization levels for COP23. The COP23 HEATT assumes a 'test and
...
other targets in the HEATT are based on the treatment targets,
...
The HEATT will allow PEPFAR teams to use country specific
...
diagnosed, linked, and start treatment. The HEATT does not
...
**Teams must not modify the structure of the COP23 HEATT in any
...
HEATT Site Tool in order to generate targets. However, this is *only*
possible for teams that do not in any way alter the structure or format
of the HEATT. Additional details are provided in COP Guidance and
will be available through COP webinars.

## Highlighted Changes from COP22 to COP23

The COP23 HEATT is largely the same as the COP21 HEATT. However,
...
based on different aspects of the HEATT that worked well and others
...
the HEATT. In turn, the HEATT targets will produce FY22 targets
...
-   Country Teamspre-validates their HEATT submission in the HEATT
    Self-Service App (available at <https://apps.datim.org/datapack/>).

-   Country Team uses HEATT Self-Service App to sync data with PAW
    Dossiers.

-   Country Team saves HEATT to SharePoint under the OU's HQ
    Collaboration \> COP 2022 - FY 2023 \> Guidance, Tools, and
    Resources folder.

-   Country Team submits a ticket in ZenDesk that includes:

    -   A link to the HEATT file saved in SharePoint
...
-   Once this ticket is received, the HEATT Support Team will confirm
...
-   The assigned PPM and the assigned DUIT Liaison use both the DataPack
    ...
    HEATT, noting any feedback in the ticket/email thread.
...
from the initial submission of a HEATT from a Country Team. While
...
ticket and email thread has been started with an initial HEATT
submission, all future HEATT submissions related to the same Country
...
-   Ensure construction of HEATT has not been tampered with.
...
-   Confirm no discrepancies between targets modeled in your submitted
    HEATT and any COP Meeting presentations to date or other
...
-   Again confirm HEATT alignment with all high-level decisions and
...
-   Confirm the HEATT is as near final as possible

**Final Submission**

-   Confirm all targets modeled in the HEATT are ready for submission
    to DATIM.
...
thread/ticket, the HEATT Support Team will move forward with
uploading your submitted HEATT to DATIM, then note completion of this
...
HEATT. Please note in addition to these regular formal submissions,
we encourage regular sharing and dialogue with Chair, PPM, and DUIT
Liaison around target setting process generally, and HEATT
specifically. Feel free to share draft versions as often as is helpful.

## HEATT SharePoint Location

The HEATT will be posted on PEPFAR SharePoint:
...
-   The file name will be "Datapack_CountryName_20220121.....". Please
    ...
    be a new version of the HEATT tool.

## Tab Categories

Each HEATT will start with 20 tabs organized in the order presented
below. Upon downloading the HEATT, the PSNUxIM tab will appear as a
