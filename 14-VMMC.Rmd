...
-   **Host Country VMMC_CIRC_SUBNAT (FY23)** $VMMC\_CIRC\_SUBNAT.T\_1$

-   **Host Country VMMC_TOTALCIRC_SUBNAT (FY23)**
    $VMMC\_TOTALCIRC\_SUBNAT.T\_1$

-   **Targeted Host Country VMMC_TOTALCIRC_SUBNAT (FY24)**
    $VMMC\_TOTALCIRC\_SUBNAT.T$

-   **Targeted Host Country VMMC_CIRC_SUBNAT (FY24)**
...
1.  Review data for the following columns, all of which come from
    ...
    a.  Host Country Estimated Male Population (FY23)
    ...
    f.  Host Country Est. PopVLS Rate (VLS/PLHIV) (FY23) (%)
...
2.  If using Spectrum as the source for Host Country Context data, the
    ...
    a.  Host Country VMMC_CIRC_SUBNAT (FY23)

    b.  Host Country VMMC_TOTALCIRC_SUBNAT (FY23)

3.  Review Host Country Estimated VMMC Coverage (FY23), which is
    calculated by dividing the FY23 Host Country Estimated
    VMMC_TOTALCIRC_SUBNAT by the FY23 Host Country Estimated Male
    Population.

4.  Review Targeted Host Country VMMC Coverage (FY24), which is
...
5.  Review modeled FY24 targets for Host Country VMMC_TOTALCIRC_SUBNAT
...
**Note: For FY24 targets, males less than 15 years old will not be
...
-   VMMC_CIRC Not Tested by PEPFAR $VMMC\_CIRC.Unk.T$
...
1.  For historical context, review FY22 results and FY23 targets for
    PEPFAR VMMC_CIRC, supplied in the DataPack as an export from data
    currently reported in DATIM.

2.  Review the FY23 estimated PEPFAR Coverage of Host Country
    VMMC_CIRC_SUBNAT, calculated by dividing FY23 PEPFAR VMMC_CIRC
    targets by the projected FY23 Host Country VMMC_CIRC_SUBNAT.

3.  Review the FY24 PEPFAR coverage of Host Country VMMC_CIRC_SUBNAT,
    which is initially set equal to the FY23 estimated coverage rate set
    in step 2, but can be adjusted as needed.

4.  For Military SNUs only, adjust the FY23 to FY24 Change in VMMC_CIRC.
    For Military SNUs, this defaults to zero, but can be adjusted to
    match strategic programming as necessary.

5.  Review FY22 results for Observed Indeterminate/Not Tested Rate and
    Observed Positivity Rate among those VMMC clients newly tested for
    HIV, both of which are obtained from DATIM.

6.  Review and adjust FY24 projections for Estimated Indeterminate/Not
...
### Total VMMC_CIRC (FY24)

For Military organization units, FY24 targets for Total VMMC_CIRC is set
...
For all other organization units, FY24 targets for Total VMMC_CIRC are
...
### VMMC_CIRC Disaggregates (FY24)

In disaggregating total VMMC_CIRC for FY24 Targets, the DataPack will
...
1.  Review VMMC_CIRC_POS linked to ART which is determined as the
    product of using Targeted ART Linkage Rate (FY24) (%) that is pulled
