...
-   **Host Country PMTCT_STAT_SUBNAT (D) - \# New ANC clients (FY23)**
...
-   **Host Country PMTCT_ART_SUBNAT (D) - \# HIV-positive pregnant women
    (FY23)** $PMTCT\_ART\_SUBNAT.D.T\_1$

-   **Host Country PMTCT_ART_SUBNAT (N) - Already on ART (FY23)**
    $PMTCT\_ART\_SUBNAT.N.Already.T\_1$

-   **Host Country PMTCT_ART_SUBNAT (N) - New on ART (FY23)**
...
1.  Review data for the following columns, all of which come from
    ...
    a.  Host Country Estimated Female Population (FY23)

    b.  Host Country Estimated PLHIV (FY23)

    c.  Host Country Estimated HIV Prevalence (FY23)

    d.  Host Country Estimated TX_CURR_SUBNAT (FY23)

    e.  Host Country Estimated ART Coverage (FY23)
...
2.  If using Spectrum as the source for Host Country Context data, the
    ...
    a.  Host Country PMTCT_STAT_SUBNAT (D) - \# New ANC clients (FY23)

    b.  Host Country PMTCT_STAT_SUBNAT (N) - Known Positive (FY23)

    c.  Host Country PMTCT_STAT_SUBNAT (N) - New Positive (FY23)

    d.  Host Country PMTCT_STAT_SUBNAT (N) - New Negative (FY23)

    e.  Host Country PMTCT_ART_SUBNAT (D) - \# HIV-positive pregnant
        women (FY23)

    f.  Host Country PMTCT_ART_SUBNAT (N) - Already on ART (FY23)

    g.  Host Country PMTCT_ART_SUBNAT (N) - New on ART (FY23)
...
1.  For historical context, review FY23 targets for PMTCT_STAT (D),
...
3.  Review FY24 targets for PMTCT_STAT (D), which is calculated by
    ...
    New ANC clients (FY23)" set in the Host Country Context section, or
    the PMTCT_STAT (D) FY23 targets from DATIM. In the case services are
    planned in FY24 where these were not provided in FY23, you may
    manually enter FY24 targets in this column.

## PMTCT: PMTCT_STAT_SUBNAT (D)

```{r echo=FALSE, results='asis'}
sheet_name <- "PMTCT"
section <- "PMTCT_STAT_SUBNAT (D)"
columns <- col_seq("V", "X")
data <- prepare_table_data(sheet_name, columns)
for (t in table_seq(data)) {
  make_table(t, section)
...
-   **PMTCT_STAT_SUBNAT (D) (FY24)** $PMTCT\_STAT\_SUBNAT.D.T$

### Instructions

1.  Review the Est. PEPFAR proportion of Host Country PMTCT_STAT_SUBNAT
    (D) (FY23) (%) that is calculated using the Host Country Context
    Section.

2.  Review Targeted PEPFAR proportion of Host Country PMTCT_STAT_SUBNAT
    (D) (FY24) (%) which will be set by default to equal the FY23
    percentage from the previous column.

3.  Review the projected target total for PMTCT_STAT_SUBNAT (D) (FY24).
...
```{r echo=FALSE, results='asis'}
sheet_name <- "PMTCT"
section <- "PMTCT_STAT (N)"
columns <- col_seq("Y", "AG")
data <- prepare_table_data(sheet_name, columns)
...
1.  Review "Targeted testing coverage of ANC clients (FY24)", which is
...
2.  Review FY22 Results for (a) Estimated % ANC1 clients with already
    Known HIV Positive status, and (b) Estimated Positivity Rate among
    Newly Tested ANC1 clients.

3.  Review FY24 projections for (a) Estimated % ANC1 clients with
    ...
    from related FY23 rates added to the Host Country Context section of
    this tab. Where these are unavailable, these instead use FY22
    ...
    different from FY22 results. See below for additional information.
...
### FY24 Projected Known Positivity and New Positivity Rates
...
```{r echo=FALSE, results='asis'}
sheet_name <- "PMTCT"
section <- "PMTCT_STAT_SUBNAT (N)"
columns <- col_seq("AH", "AK")
data <- prepare_table_data(sheet_name, columns)
for (t in table_seq(data)) {
  make_table(t, section)
  }
...
1.  Review the Total PMTCT_STAT_SUBNAT (N) that is calculated using the
    target set in the previous section for PMTCT_STAT_SUBNAT (D) (FY24)
    and Targeted testing coverage of ANC1 clients (FY24) (%) from the
    PMTCT_STAT (N) section. This will be used to calculate the FY24
    targets for the remainder of this section

2.  Review Host Country PMTCT_STAT_SUBNAT (N) - Known Positive (FY24)
...
3.  Review Host Country PMTCT_STAT_SUBNAT (N) - New Positive (FY24) in
    the same manner as it uses the SUBNAT Numerator and Est. Positivity
    Rate among Newly Tested ANC1 clients (FY24) (%). Adjust this
    percentage from the previous section to make changes to this target.

4.  Review Host Country PMTCT_STAT_SUBNAT (N) - New Negative (FY24)
    which will be calcuated as the remainder of Host Country
    PMTCT_STAT_SUBNAT (N) - Known Positive (FY24), less Host Country
    PMTCT_STAT_SUBNAT (N) - Known Positive (FY24) and Host Country
    PMTCT_STAT_SUBNAT (N) - New Positive (FY24).
...
2.  Review modeled targets for PMTCT_ART (N) Already on ART. For the
    purposes of COP21 target setting in the DataPack, FY24 targets for
...
```{r echo=FALSE, results='asis'}
sheet_name <- "PMTCT"
section <- "PMTCT_ART_SUBNAT"
columns <- col_seq("AO", "AQ")
data <- prepare_table_data(sheet_name, columns)
for (t in table_seq(data)) {
  make_table(t, section)
  }
...
-   **Est. Host Country \# HIV-positive pregnant women (FY24)**
    $PMTCT\_ART\_SUBNAT.D.T$

-   **Est. Host Country \# HIV+ Pregnant Women Already on ART (FY24)**
    $PMTCT\_ART\_SUBNAT.N.Already.T$

-   **Est. Host Country \# HIV+ Pregnant Women New on ART (FY24)**
    $PMTCT\_ART\_SUBNAT.N.New.T$

### Instructions

1.  Review Est. Host Country \# HIV-positive pregnant women (FY24). This
    is the summation of FY24 Targets set in the PMTCT_STAT_SUBNAT (N)
...
```{r echo=FALSE, results='asis'}
sheet_name <- "PMTCT"
section <- "HTS_TST: PMTCT Post ANC1"
columns <- col_seq("AR", "AX")
data <- prepare_table_data(sheet_name, columns)
for (t in table_seq(data, max_col = 4)) {
  make_table(t, section)
  }
